...
-------------------------------------------------------------------
  이 파일은 아래 주제를 포함하는 라이브러리 클래스를 포함하고 있습니다.
  1) Arduino
  2) LiDAR
  3) Camera
-------------------------------------------------------------------
  ...
  수정일: 2022-11-18
-------------------------------------------------------------------
  코드에 문제가 있다면 메인 저자(Jonghun Kim)에게 연락해주세요.
-------------------------------------------------------------------
  워크숍 동안 이 파일을 수정하지 마세요.
-------------------------------------------------------------------
"""
...
class libARDUINO(object):
    def __init__(self):
        self.port = None
        self.baudrate = None
        self.wait_time = WAIT_TIME  # 초 단위

    # Arduino 시리얼 USB 포트 설정
    ...
    def edge_detection(self, img, width=0, height=0, gap=0, threshold=0, print_enable=False):
        ...
        if lines is not None:
            ...
            for line in lines:
                ...
                # x 범위: 0 ~ self.col / y 범위: 0 ~ self.row
            ...
            if print_enable:
                if prediction is not None:
                    print("차량 방향: ", DIRECTION[prediction])
                self.image_show(replica)
